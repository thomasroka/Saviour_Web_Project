...
const SigninForm = () => {
    ...
    return (
        <div className='w-full'>
            <div>
                <h2 className='text-3xl font-bold bg-gradient-to-r from-slate-900 to-slate-600 bg-clip-text text-transparent'>Welcome Back</h2>
                <p className='text-sm mt-2 text-slate-500'>
                    Don't have an account? <Link to="/signup" className='font-semibold text-indigo-600 hover:text-indigo-700 transition-colors'>Create an Account</Link>
                </p>
            </div>
            ...
            <form onSubmit={(e) => { e.preventDefault(); signinHandle(); }} className='mt-6 flex flex-col gap-4'>
                ...
                <div className='flex flex-row items-center gap-2.5 mt-1'>
                    <input type="checkbox" id="terms" className='w-4 h-4 rounded cursor-pointer accent-indigo-600' />
                    <label htmlFor="terms" className='text-sm cursor-pointer text-slate-500'>I agree to Terms and conditions.</label>
                </div>
                <button
                    type="submit"
                    disabled={loading}
